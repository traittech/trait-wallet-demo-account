...
async function create_nft_collections(
  ...
  try {
    logger.info("Start to create NFT Collections for the AppAgent ID " + appAgentId);
...
async function set_metadata_and_mint_nft(
  ...
  try {
    logger.info(
      "Start to configure NFT Collection `" + collectionData.collectionId + "` for the AppAgent ID " + appAgentId,
    );
    ...
    const set_collection_metadata_action = [{ AppAgentId: appAgentId }, set_collection_metadata_call];
    ...
    for (const nftToken of collectionData.nftTokens) {
      ...
      const mint_nft_call = api.tx.nfts.mint(collectionData.collectionId, tokenId, token_recipient);
      const mint_nft_action = [{ AppAgentId: appAgentId }, mint_nft_call];
      const set_metadata_call = api.tx.nfts.setMetadata(collectionData.collectionId, tokenId, metadataUrl);
...
export { create_nft_collections, create_nft_transfers, set_metadata_and_mint_nft };
